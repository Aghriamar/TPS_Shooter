...
void AWeaponDefault::ClipDropTick(float DeltaTime)
{
	if (DropClipFlag)
	{
		...
		{
			DropClipFlag = false;
			InitDropMesh_OnServer_Implementation(WeaponSetting.ClipDropMesh.DropMesh,
...
void AWeaponDefault::ShellDropTick(float DeltaTime)
{
	if (DropShellFlag)
	{
		...
		{
			DropShellFlag = false;
			InitDropMesh_OnServer_Implementation(WeaponSetting.ShellBullets.DropMesh,
...
{
	// Создаем правила для прикрепления. SnapToTarget означает, что компонент "прилипнет"
	// к родителю, используя свое относительное положение, которое задали в редакторе.
	// Второй параметр (true) "сваривает" физические тела вместе, что обычно и требуется.
	//const FAttachmentTransformRules AttachmentRules(EAttachmentRule::SnapToTarget, false);
	if (SkeletalMeshWeapon && SkeletalMeshWeapon->SkeletalMesh)
	{
		ActiveWeaponComponent = SkeletalMeshWeapon;
		ShootLocation->SetupAttachment(ActiveWeaponComponent);
		UE_LOG(LogTemp, Log, TEXT("ShootLocation attached to SkeletalMeshWeapon"));
	}
	else if (StaticMeshWeapon && StaticMeshWeapon->GetStaticMesh())
	{
		ActiveWeaponComponent = StaticMeshWeapon;
		ShootLocation->SetupAttachment(ActiveWeaponComponent);
		UE_LOG(LogTemp, Log, TEXT("ShootLocation attached to StaticMeshWeapon"));
	}
	else
	{
		ActiveWeaponComponent = RootComponent; // Запасной вариант
		ShootLocation->SetupAttachment(ActiveWeaponComponent);
...
void AWeaponDefault::SetWeaponStateFire_OnServer_Implementation(bool bIsFire)
{
	if (CheckWeaponCanFire())
	{
		...
		WeaponFiring = false;
		FireTimer = 0.01; //ToDo should best option create delay before fire and delay before InitFirstFire(remove if player click so fast like sniper rifle shoot like machine gun)
}
...
void AWeaponDefault::Fire()
{
	...
	{
		AnimWeaponStart_Multicast_Implementation(WeaponSetting.AnimWeaponInfo.AnimWeaponFire);
	}
	...
	FXWeaponFire_Multicast_Implementation(WeaponSetting.EffectFireWeapon, WeaponSetting.SoundFireWeapon);
	...
	if (ShootLocation)
	{
		...
		for (int8 i = 0; i < NumberProjectile; i++)//Shotgun
		{
			...
			if (ProjectileInfo.Projectile)
			{
				...
				if (myProjectile)
				{
					myProjectile->InitProjectile(WeaponSetting.ProjectileSetting); //InitialLifeSpan = 20.0f;
				}
			}
			else
			{
...
void AWeaponDefault::InitReload()
{
		...
		&& SkeletalMeshWeapon->GetAnimInstance())
	{
		...
		AnimWeaponStart_Multicast_Implementation(AnimWeaponToPlay);
	}
...
void AWeaponDefault::InitDropMesh_OnServer_Implementation(UStaticMesh* DropMesh, FTransform Offset, FVector DropImpulseDirection, float LifeTimeMesh, float ImpulseRandomDispersion, float PowerImpulse, float CustomMass)
{
	if (DropMesh)
	{
		...
		ShellDropFire_Multicast_Implementation(DropMesh, Transform, DropImpulseDirection, LifeTimeMesh, ImpulseRandomDispersion, PowerImpulse, CustomMass, LocalDir);
	}
}
